...
class ProcessManager:
    ...
    def start_process(self, process_type, config):
        ...
        try:
            ...
            elif process_type == 'KAFKA_RESUME':
                from apps.services.kafka_resume import KafkaResume
                process = Process(target=KafkaResume().run)
            else:
    ...
    def start_all_processes(self):
        ...
        for process_type, configs in self.process_configs.items():
            for config in configs:
                print(f"process_type: {process_type}, config: {config}")
