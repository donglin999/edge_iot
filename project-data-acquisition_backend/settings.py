...
class DevelopmentConfig(Config):
    ...
    INFLUXDB_HOST = '10.60.11.202'
    INFLUXDB_PORT = 8086
    INFLUXDB_TOKEN = 'jWpPP9weV8gXcXhEeuFCXmJU57NKv47uYxQo0rUE40dr9BtuJBo9S3V8RW-iZghpXAIAPtKjIv9JqY8vDbXFjg=='
    INFLUXDB_ORG = "IMRC"
    INFLUXDB_BUCKET = "KSRG"
    ...
    API_TAG = "API_KAFKA"
    # 缓存SQLite配置
    SQLITE_ENABLED = True  # 是否启用SQLite缓存
    SQLITE_DB_PATH = os.path.join(basedir, 'database/kafka_buffer.sqlite3')  # SQLite数据库文件路径
    SQLITE_TABLE_NAME = "kafka_buffer"  # 数据表名
    
    # 断点续传配置
    KAFKA_RESUME_ENABLED = True  # 是否启用断点续传
    KAFKA_RESUME_INTERVAL = 5  # 续传检查间隔(秒)
    KAFKA_RESUME_BATCH_SIZE = 100  # 每批处理的消息数量
    KAFKA_RESUME_RETENTION_DAYS = 7  # 数据保留天数
