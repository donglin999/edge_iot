...
class OPCDataRetriever:
    def __init__(self, server_url, node_id, node_id2, node_id3):
    ...
    def save_to_csv(self, data, filename='opc_data.csv'):
        df = pd.DataFrame(data, columns=['time', 'stress', 'speed', 'position'])
    ...
    def run(self, interval=0.1, duration=60):  # interval in seconds, duration in seconds
        ...
        while (time.time() - start_time) < duration:
            ...
            value2 = self.read_node_value(self.node_id2)
            value3 = self.read_node_value(self.node_id3)
            if value is not None:
                timestamp = time.strftime('%Y-%m-%d %H:%M:%S.%f', time.localtime(time.time())) + str(time.time() % 1)[2:5]  # Get current time with milliseconds
                data.append([timestamp, value, value2, value3])
            time.sleep(interval)
...
if __name__ == "__main__":
    ...
    node_id2 = 'ns=1;s=io_sig.vi_flt_2'
    node_id3 = 'ns=1;s=io_sig.ai_flt_2'
    opc_retriever = OPCDataRetriever(server_url, node_id, node_id2, node_id3)
